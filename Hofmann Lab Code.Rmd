...
```{r}
tris <-"8.19.2021-1_lol-tris.csv" # values collected during calibration
raw <- "8.19.2021-1_lol.csv" # values collected in field
filename <- "8.19.2021" # add file name for final data frame
```
...
omegas_ph_tris <- data.frame(trisdata$date_time_lubridate,omegas_temp_tris$durafet,omegas_temp_tris$v1) %>% # create data frame with durafet therm and voltage 1 values
  ...
  mutate(tempdiff=abs(omegas_temp_tris.durafet-lag(omegas_temp_tris.durafet,n=1,default=first(omegas_temp_tris.durafet)))) %>% # find difference between two temp measurements 5 rows apart to identify where temperature stabilizes
    mutate(phdiff=abs(trisph-lag(trisph,n=1,default=first(trisph)))) # find difference between two pH measurements 3 rows apart to identify where pH stabilizes
```
