...
omegas_v <- omegas %>% # create new dataframe using omegas values
  ...
  mutate(sh_a=0.00106329736674527) %>% # create column with Steinhart constant A
  mutate(sh_b=0.000251377462346306) %>% # create column with Steinhart constant B
  mutate(sh_c=2.55455247726963E-08) %>% # create column with Steinhart constant C
  mutate(k=(1/(sh_a+sh_b*log(rthermistor)+sh_c*(log(rthermistor))^3))) %>% # create column to calculate intermediate value K
  mutate(durafet=(k-273.15)) # create new column to calculate durafet therm 
```
...
```{r}
omegas_calibration %>% # work in omegas_calibration dataframe
  filter(row_number()!=c(1:150)) %>% # remove the first 150 rows (first 30 minutes of data)
  filter(omegas_cleaned,phdiff==min(phdiff)) %>% # select the row that contains the minimum value of change in pH over time and add it alone to a new data frame
  # THIS WAS WORKING YESTERDAY; WHY NOT TODAY?
  # ERROR MESSAGE: "omegas_v.durafet must be a logical vector, not a double"
