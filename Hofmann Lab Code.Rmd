...
### Importing data
```{r}
trisdata <- read_csv(tris) %>% # read in tris data excel file
...
## Importing raw data
```{r}
rawdata <- read_csv(raw) %>% # read in raw data excel file
  filter(row_number()!=c(1:5)) %>% # remove the first 5 rows
  row_to_names(row_number=1) %>% # make the top row the column headers
  rename("Date"=1,
         "Time"=2) %>%
...
write.csv(raw_calibrated, "processed_data.csv", row.names = FALSE)
```