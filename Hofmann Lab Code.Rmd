...
omegas_calibration <- data.frame(omegas_v$durafet,omegas_v$v1) %>% # create data frame with durafet therm and voltage 1 values
  ...
  mutate(tempdiff=omegas_v.durafet-lag(omegas_v.durafet,n=3,default=first(omegas_v.durafet))) %>% # find difference between two temp measurements 5 rows apart to identify where temperature stabilizes
    mutate(phdiff=omegasph-lag(omegasph,n=3,default=first(omegasph))) %>% # find difference between two pH measurements 5 rows apart to identify where pH stabilizes
  filter(row_number()!=c(1:3)) %>% # remove the first 3 rows (first 30 minutes of data)
  mean(tempdiff) # calculate the mean of the difference in temperature
  sd(tempdiff) # calculate the standard deviation of the difference in temperature
  mean(phdiff) # calculate the mean of the difference in pH
  sd(phdiff) # calculate the standard deviation of the difference in pH
```