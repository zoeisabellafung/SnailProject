...
trisdata <- read_csv(here("data","7.12.2021-tris.csv")) %>%
  ...
  slice(1:(n()-12))) %>% # remove firsr 2 minutes of data (12 rows)
  filter(row_number()!=c())
...
t <- as.numeric(rep(k+temp),length(temp))
...
```{r}
omegas_cleaned <- omegas_calibration %>% # work in omegas_calibration dataframe
