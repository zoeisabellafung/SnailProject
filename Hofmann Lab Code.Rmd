...
```{r}
tris <-"9.16.2021-1_tris.csv" # values collected during calibration
raw <- "9.16.2021-1_lol.csv" # values collected in field
filename <- "9.16.2021" # add file name for final data frame
```

# Calibration with tris
## Importing data
```{r}
...
omegas_ph_tris <- data.frame(trisdata$date_time_lubridate,omegas_temp_tris$durafet,omegas_temp_tris$v1) %>% # create data frame with durafet therm and voltage 1 values
  ...
  mutate(tempdiff=abs(omegas_temp_tris.durafet-lag(omegas_temp_tris.durafet,n=4,default=first(omegas_temp_tris.durafet)))) %>% # find difference between two temp measurements 5 rows apart to identify where temperature stabilizes
    mutate(phdiff=abs(trisph-lag(trisph,n=4,default=first(trisph)))) # find difference between two pH measurements 4 rows apart to identify where pH stabilizes
```

### Choose calibration values
```{r}
calibration_values <- omegas_ph_tris %>% # work in omegas_calibration dataframe
  filter(row_number()!=c(1:50)) %>% # remove the first 100 rows (first 10 minutes of data)
...
```{r}
omegas_ph_raw <- data.frame(rawdata$date_time_lubridate,omegas_temp_raw$durafet,omegas_temp_raw$v1) %>% # create data frame with durafet therm and voltage 1 values
...
write.csv(raw_calibrated, paste(filename,".csv", sep=""), row.names = FALSE) # create csv with final processed data
write.csv(calibration_values,paste(filename,"_calibration.csv",sep=""),row.names=FALSE) # create csv with calibration values
```