...
```{r}
tris <-"7.12.2021-tris.csv" #values collected during calibration
raw <- "7.12.2021-raw.csv" #values collected in field
```

### Importing data
```{r}
trisdata <- read_csv(here("data",tris)) %>% # read in tris data excel file
...
## Importing raw data
```{r}
rawdata <- read_csv(here("data",raw)) %>% # read in raw data excel file TIME COLUMN ISNT READING IN RIGHT
  filter(row_number()!=c(1:5)) %>% # remove the first 5 rows
  row_to_names(row_number=1) %>% # make the top row the column headers
  rename("Date"=1,
         "Time"=2) %>%
  ...
  slice(1:(n()-12)) %>% # remove first 2 minutes of data (12 rows)
  mutate(date_lubridate=mdy(date)) %>% # tell R "date" is a DATE
  mutate(time_lubridate=hms(time)) %>% # tell R "time" is a TIME HAD TO EDIT THIS BC COLUMN NOT TITLED RIGHT
  unite(date_time, c(date,time), sep = " ", remove = TRUE) %>% # combine the date and time columns
  mutate(date_time_lubridate=mdy_hms(date_time)) # tell R "date_time" is a DATE and a TIME
```
...
omegasph_raw <- data.frame(rawdata$date_time_lubridate,omegas_raw$durafet,omegas_raw$v1) %>% # create data frame with durafet therm and voltage 1 values
  ...
  mutate(st=(8.31451*tk/96487*log(10))) %>% # add S(T) column to data frame
  mutate(eo=((calibration_values$omegas_v.v1)-(calibration_values$omegasph)*8.31451*(273.15+calibration_values$omegas_v.durafet)*log(10)/96487)) %>% # calculate eo value using calibration values
...
```{r}
raw_calibrated <- omegasph_raw %>% # create new dataframe using omegasph_raw data
  select(date_time,omegas_raw.durafet,omegasrawph) %>% # keep only date/time, temperature, and pH columns
  rename("Date, Time"=1,
         "Temperature"=2,
         "pH"=3)

write.csv(raw_calibrated, here("data", "processed_data.csv"), row.names = FALSE)
```